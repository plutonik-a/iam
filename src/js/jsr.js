...
var bySelector = function (selector) {
 ...
 */
var configs = ["/content/data/layout_jsr.json", "/content/data/all-items.json", "/content/data/der_bau.json", "/content/data/uebungen/layout_1.json", "/content/data/uebungen/layout_2.json", "/content/data/uebungen/layout_3.json"];
...
function initialiseView() {
    zeitdokumente = byId("zeitdokumente");
    einfuehrungstext = byId("einfuehrungstext");
    verknuepfungen = byId("verknuepfungen");
    objekt = bySelector(".object");
...
function removeAllMediaboxes() {
    var mediaboxes = document.querySelectorAll('.mediabox');
    var i = 0;
    var l = mediaboxes.length;
    for (; i < l; i++) {
        medienverweise.removeChild(mediaboxes[i]);
    }
}
...
function loadContentAndCreateLayout() {
    ...
    xhr("GET", ((currentConfigId && configs) ? configs[currentConfigId] : "/content/data/layout_jsr.json"), null, function (xmlhttp) {
        ...
        for (var i = 0; i < jsonContent.content_items.length; i++) {
            ...
            switch (currentItem.type) {
                ...
                case "verknuepfungen":
                    ...
                    break;
                case "objekt":
                    createObject(currentItem);
                    break;
                case "merkliste":
                    createMerkliste(currentItem);
                    break;
                case "medienverweise":
 ...
 * @param contentItem
 */
function createObject(contentItem) {
 ...
 * @param contentItem
 */
function createMerkliste(contentItem) {
    console.log("createMerkliste()");
    show(merkliste);
}
...
function createEinfuehrungstext(contentItem) {
    ...
    xhr("GET", contentItem.src, null, function (xmlhttp) {
        console.log("received response for textauszug", xmlhttp.responseText);
        einfuehrungstext.querySelector(".contentfragment").innerHTML = xmlhttp.responseText;
    });
}
...
function createTextauszug(contentItem) {
    ...
    xhr("GET", contentItem.src, null, function (xmlhttp) {
        console.log("received response for textauszug", xmlhttp.responseText);
        excerpt.querySelector(".contentfragment").innerHTML = xmlhttp.responseText;
    });
}
 ...
 * @param item
 */
function createMedienverweise(item) {
    console.log("createMedienverweise()");

    // Box bauen
    var box = document.createElement('article');
    box.classList.add('mediabox', 'box', 'media');
    ...
    for (var i = 0; i < item.content.length; i++) {
        ...
        li.appendChild(a);
        // set the target from the current list element as href
        a.href = item.content[i].target;
        // and set the title as textContent
        a.textContent = item.content[i].title;
        // append the complete li element as child to ul
        ul.appendChild(li);
    }
    medienverweise.appendChild(box);
}
